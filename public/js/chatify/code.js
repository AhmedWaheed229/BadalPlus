...
//   post_id = $("meta[name=url]").attr("data-post_id"),
...
const getPostId = () => $("meta[name=post]").attr("content");
const getMessengerType = () => $("meta[name=type]").attr("content");
const setMessengerId = (id) => $("meta[name=id]").attr("content", id);
const setPostId = (id) => $("meta[name=post]").attr("content", id);
...
function updateSelectedContact(user_id, post_id) {
    $(document).find(".messenger-list-item").removeClass("m-list-active");
    $(document)
        .find(
            ".messenger-list-item[data-contact=" + (user_id || getMessengerId()) + "][data-post_id=" + (getPostId()) + "]"
        )
...
function sendTempMessageCard(message, id, post_id = 0) {
    console.log('posttttt ' + post_id);

    return `
<div class="message-card mc-sender" data-post_id="${post_id}"  data-id="${id}">
  <p>
      ${message}
      <sub>
...
function errorMessageCard(id, error = "") {
    messagesContainer
        ...
        .addClass("mc-error");
    messagesContainer.find(".message-card[data-id=" + id + "] p").html(error)
    messagesContainer
        .find(".message-card[data-id=" + id + "]")
        .find("svg.loadingSVG")
        .remove();
 ...
 *-------------------------------------------------------------
 */
function IDinfo(id, type, post_id) {
    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $(".messenger-infoView")
                    .find(".avatar")
                    .css("background-image", 'url("' + data.fetch.image_url + '")');
                $(".header-avatar").css(
                    "background-image",
                    'url("' + data.fetch.image_url + '")'
                );
                // Show shared and actions
                $(".messenger-infoView-btns .delete-conversation").show();
                $(".messenger-infoView-shared").show();
                // fetch messages
                fetchMessages(id, type, true, post_id);
        ...
        $.ajax({
            ...
            beforeSend: () => {
                ...
                if (hasFile) {
                    ...
                    messagesContainer
                        .find(".messages")
                        .append(sendTempMessageCard(inputValue, tempID, getPostId()));
                }
                // scroll to bottom
            ...
            success: (data) => {
                if (data.post) {
                    if (!data.post.status == 0) {
                        var img = data.post.image_url;
                        var title = data.post.title;
                        var currency = data.post.currency ? data.post.currency.name : "";
                        var price = data.post.price + " " + currency;
                        var convert_price = data.convert_price;
                        $(".post-data").find('img').attr('src', img);
                        $(".post-data").find('h4').html(title);
                        $(".post-price").find('h4.main_price').html(price);
                        $(".post-price").find('h4.convert_price').html(convert_price);
                        $(".post-info").removeClass('d-none');
                        $(".messenger-sendCard form").removeClass('d-none');
                    } else {
                        $(".messenger-sendCard form").addClass('d-none');
                        $(".messenger-sendCard").append('<h6 class="text-center soldout text-danger">Sold Out</6>');
                        $(".post-info").addClass('d-none');
                    }
                } else {
                    $(".post-info").addClass('d-none');
                }
                ...
                if (data.post_user != "") {
                    $(".post-info").find("." + data.post_user.status).siblings().removeClass('d-flex').addClass('d-none');
                    $(".post-info").find("." + data.post_user.status).removeClass('d-none').addClass('d-flex')
                        .find(".price").html(data.post_user.price + ' ' + data.post_user.name);

                    if ((data.post_user.status == "escrow") && (auth_id == data.post.created_by)) {
                        $(".post-info").find("." + data.post_user.status).find("button").addClass('d-inline-block').removeClass('d-none');
                    } else if ((data.post_user.status == "escrow") && (auth_id != data.post.created_by)) {
                        $(".post-info").find("." + data.post_user.status).find("button").addClass('d-none').removeClass('d-inline-block');
                    }

                    if ((data.post_user.status == "hold") && (auth_id != data.post.created_by)) {
                        $(".post-info").find("." + data.post_user.status).find("button").removeClass('d-none').addClass('d-inline-block');
                    } else if ((data.post_user.status == "hold") && (auth_id == data.post.created_by)) {
                        $(".post-info").find("." + data.post_user.status).find("button").removeClass('d-inline-block').addClass('d-none');
                    }
                } else {
                    if (auth_id != data.post.created_by) {
                        $(".post-info").find(".start-escrow").siblings()
                            .removeClass('d-block').addClass('d-none');
                        $(".post-info").find(".start-escrow")
                            .removeClass('d-none').addClass('d-block');
                    } else {
                        $(".post-info .start-escrow").removeClass('d-block').addClass('d-none').siblings().removeClass('d-block').addClass('d-none');
                    }
                }
                ...
                if (data.error > 0) {
                    ...
                    // update contact item
                    updateContactItem(getMessengerId(), getPostId());
...
function fetchMessages(id, type, newFetch = false, post_id) {
    if (newFetch) {
        messagesPage = 1;
    ...
    if (messenger != 0 && !noMoreMessages && !messagesLoading) {
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: (data) => {
                $(".messenger-sendCard .soldout").remove();
                if (data.post) {
                    if (!data.post.status == 0) {
                        var img = data.post.image_url;
                        var title = data.post.title;
                ...
                if (data.post_user != "") {
                    ...
                    if (auth_id != data.post.created_by) {
                        $(".post-info .start-escrow").removeClass('d-none').addClass('d-flex');
                        $(".post-info .start-escrow").siblings().removeClass('d-flex').addClass('d-none');
                    } else if (auth_id == data.post.created_by) {
                        $(".post-info .start-escrow").removeClass('d-flex').addClass('d-none');
                        $(".post-info .start-escrow").siblings().removeClass('d-flex').addClass('d-none');
                    }
                }
...
channel.bind("messaging", function (data) {
    ...
    if (data.from_id == getMessengerId() && data.to_id == auth_id && data.post_id == getPostId()) {
        if (data.post) {
            if (!data.post.status == 0) {
                var img = data.post.image_url;
                var title = data.post.title;
        ...
        scrollToBottom(messagesContainer);
        makeSeen(true, data.post_id);

        // remove unseen counter for the user from the contacts list
        $(".messenger-list-item[data-contact=" + getMessengerId() + "][data-post_id=" + getPostId() + "]")
            .find("tr>td>b")
            .remove();
    }
});
...
clientListenChannel.bind("client-contactItem", function (data) {
    if (data.update_for == auth_id) {
        data.updating == true
            ? updateContactItem(data.update_to, data.post_id)
 ...
 *-------------------------------------------------------------
 */
function makeSeen(status, post_id) {
    if (document?.hidden) {
        return;
    }
    // remove unseen counter for the user from the contacts list
    ...
    $.ajax({
        url: url + "/makeSeen",
        method: "POST",
        data: {_token: access_token, id: getMessengerId(), post_id: post_id},
        dataType: "JSON",
    });
 ...
 *-------------------------------------------------------------
 */
function updateContactItem(user_id, post_id) {
    if (user_id != auth_id) {
        let listItem = $("body")
            .find(".listOfContacts")
            .find(".messenger-list-item[data-contact=" + user_id + "][data-post_id=" + post_id + "]");
        $.ajax({
            ...
            success: (data) => {
                ...
                // update data-action required with [responsive design]
                cssMediaQueries();
                updateSelectedContact(user_id, post_id);
            },
            error: () => {
...
$(document).ready(function () {
    // get contacts list
    getContacts();
    if (getMessengerId() > 0 && getPostId() > 0) {
        fetchMessages(getMessengerId(), getMessengerType(), false, getPostId());
    }
    ...
    pusher.connection.bind("state_change", function (states) {
        ...
        channel.bind("pusher:subscription_succeeded", function () {
            ...
            if (getMessengerId() != 0) {
                ...
                IDinfo(getMessengerId(), getMessengerType(), getPostId());
            }
        });
    });
    ...
    $("body").on("click", ".messenger-list-item", function () {
        ...
        const postID = $(this).attr("data-post_id");
        routerPush(document.title, `${url}/${userID}/${postID}`);
        updateSelectedContact(userID, postID);
    });
    ...
    $("body").on("click", ".messenger-list-item", function () {
        ...
        const postId = $(this).find("p[data-post_id]").attr("data-post_id");

        const dataType = $(this).find("p[data-type]").attr("data-type");
        setMessengerId(dataId);
        setPostId(postId);
        setMessengerType(dataType);
        IDinfo(dataId, dataType, postId);
    });
    ...
    $("body").on("click", ".favorite-list-item", function () {
        ...
        const postid = $(this).find("div.avatar").attr("data-post_id");

        setMessengerId(uid);
        setPostId(postid);
        setMessengerType("user");
        IDinfo(uid, "user", postid);
        updateSelectedContact(uid, postid);
...
$(".post-info .start-escrow button").click(function () {
    let tempID = `temp_${temporaryMsgId}`;
    ...
    formData.append("temporaryMsgId", tempID);
    formData.append("_token", access_token);
    formData.append("message", "escrow");
    ...
    $.ajax({
        ...
        beforeSend: () => {
            ...
            messagesContainer
                .find(".messages")
                .append(sendTempMessageCard("starting escrow", tempID, getPostId()));
...
$(".post-info .escrow button").click(function () {
    let tempID = `temp_${temporaryMsgId}`;
    let formData = new FormData($("#message-form")[0]);
    formData.append("id", getMessengerId());
    formData.append("post_id", getPostId());
    ...
    $.ajax({
        ...
        beforeSend: () => {
            ...
            messagesContainer
                .find(".messages")
                .append(sendTempMessageCard("hold", tempID, getPostId()));
...
$(".post-info .hold button").click(function () {
    let tempID = `temp_${temporaryMsgId}`;
    let formData = new FormData($("#message-form")[0]);
    formData.append("id", getMessengerId());
    formData.append("post_id", getPostId());
    ...
    formData.append("temporaryMsgId", tempID);
    formData.append("_token", access_token);
    formData.append("message", "release");
